...
export const updateBankAccount = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const bankAccount = user.banks.id(bankId);
        if (!bankAccount) {
...
export const removeBankAccount = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        console.log("2. Request received for userId:", userId, "and bankId:", bankId);
        ...
        user.banks.pull({ _id: bankId }); 
        
        await user.save();
        
        console.log("9. User saved to database successfully. Final banks count:", user.banks.length);
        ...
        console.error("10. An unhandled error occurred in removeBankAccount:", error);
        next(error);
    }
};
...
import TermsPolicyModel from "../models/termspolicy.model";

// Routes
export const getTermPolicies = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        // Find one policy, sorted by the most recent if needed
        const policy = await TermsPolicyModel.findOne().sort({ createdAt: -1 });
...
export const createTermPolicy = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const policy = new TermsPolicyModel(req.body)
        await policy.save();
...
export const editTermPolicy = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { id } = req.params;
        ...
        const existingPolicy = await TermsPolicyModel.findById(id);
...
export const deleteTermPolicy = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { id } = req.params;
        const policy = await TermsPolicyModel.findByIdAndDelete(id);

...
export const updateListing = CatchAsyncError(
        ...
        for (let i = 0; i < hotel_room_types.length; i++) {
          ...
          if (roomType.images && Array.isArray(roomType.images)) {
            ...
            for (const image of roomType.images) {
              if (!image) {
                console.warn(
                  `Undefined image encountered in room_images for room type ${i}`
                );
                continue;
              }
              ...
              if (typeof image === "string") {
                ...
                console.warn(
                  `Invalid image format encountered in room_images for room type ${i}`
                );
              }
            }
...
export const getAllHostListingsById = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
...
export const deleteListing = async (
  ...
  try {
    ...
    res.status(200).json({
      ...
      }' has been suspended and is scheduled for permanent deletion on ${twoWeeksFromNow.toLocaleDateString()}.`,
      data: updatedListing,
    });
  } catch (error: any) {
...
export const approveListing = async (
  ...
  try {
    ...
    const updated = await listingModel.findByIdAndUpdate(id, updatedData, {
      new: true,
    });
    res.status(201).json({
      success: true,
...
export const postReview = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { cleanliness, customer_service, overall_rating, comment } =
        req.body;
...
export const createAttraction = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    console.log("Received data:", {
      ...
      latitude,
      longitude,
      image: image ? "Image data received" : "No image",
    });
    ...
    if (listing.attractions.length >= 2) {
      return res
        .status(400)
        .json({ message: "Maximum number of attractions (2) already reached" });
    }
...
// Update attraction
export const editAttraction = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const attractionIndex = listing.attractions.findIndex(
      (attr: any) => attr._id.toString() === attractionId
    );
...
// Delete attraction
export const deleteAttraction = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { listingId, attractionId } = req.params;
      ...
      const attraction = (listing.attractions as any).id(attractionId);
      if (!attraction) {
        return res.status(404).json({ message: "Attraction not found" });
      }
      ...
      (listing.attractions as any).pull(attractionId);
      await listing.save();
      ...
      res
        .status(500)
        .json({ message: "Error deleting attraction", error: error.message });
    }
  }
);
...
export const getAttractions = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { listingId } = req.params;
      ...
      res
        .status(500)
        .json({ message: "Error fetching attractions", error: error.message });
    }
  }
);
...
export const getListingReports = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const suspendedListings = await listingModel.countDocuments({
        isSuspend: true,
      });
...
// search listing
export const searchListing = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const {
        type,
      ...
      if (guests) {
        if (type === "hotel") {
          query["hotel_room_types.max_occupancy"] = {
            $gte: parseInt(guests as string, 10),
          };
        } else {
          if (type === "apartment") {
            query["apartment_availability.maximum_nights"] = {
              $gte: parseInt(guests as string, 10),
            };
          }
        }
      ...
      if (minPrice) {
        priceQuery[Object.keys(priceQuery)[0]].$gte = parseFloat(
          minPrice as string
        );
      }
      if (maxPrice) {
        priceQuery[Object.keys(priceQuery)[0]].$lte = parseFloat(
          maxPrice as string
        );
      }
      ...
      if (checkIn && checkOut) {
        ...
        if (type === "hotel") {
          query["hotel_room_types.availability"] = {
            $all: [{ $elemMatch: { $gte: checkInDate, $lt: checkOutDate } }],
          };
        } else {
          // For apartments

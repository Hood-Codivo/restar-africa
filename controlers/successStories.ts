...
export const updateSuccessStory = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!updatedStory) {
            return res.status(400).json({ success: false, message: 'Story could not be updated' });
        }
...
export const deleteSuccessStory = async (req: Request, res: Response) => {
    try {
        ...
        if (error.name === 'CastError') {
            res.status(400).json({ success: false, message: 'Invalid story ID format' });
        } else {
...
export const createLikes = async (req: Request, res: Response) => {
    try {
        ...
        const hasLiked = story.likedBy.some(id => id.toString() === userIdStr);

        if (hasLiked) {
            story.likes -= 1;
            story.likedBy = story.likedBy.filter(id => id.toString() !== userIdStr);
        } else {
            story.likes += 1;
...
export const getStoriesByUserId = async (req: Request, res: Response) => {
    try {
        ...
        if (!successStories) {
            return res.status(404).json({ message: 'No success stories found for this user.' });
        }

        res.status(200).json(successStories);
    } catch (error) {
        res.status(500).json({ message: 'Failed to retrieve success stories.', error: error.message });
    }
};



export const getSuccessStory = async (req: Request, res: Response, next: NextFunction) => {

...
export const getAllWithdrawRequests = async (req: Request, res: Response) => {
  try {
    const withdrawRequests = await withdrawRequestModels
      .find()
      .sort({ createdAt: -1 });
  ...
  try {
    ...
    if (status === "Approved") {
      ...
      user.payouts.push({
        ...
        date: new Date(),
        status: withdrawRequest.status as
          | "Cancelled"
          | "Rejected"
          | "Approved"
          | "Pending",
        description: `Withdrawal of ${withdrawRequest.amount}`,
        currency: "NGN",
      });
    ...
    await Notification.create({
      ...
      content: `Your withdraw request for $${
        withdrawRequest.amount
      } has been ${status.toLowerCase()}${reason ? `. Reason: ${reason}` : ""}`,
    });
...
export const downloadRecentPayouts = async (req: Request, res: Response) => {
  try {
    ...
    const recentPayouts = user.payouts.filter(
      (payout: any) => new Date(payout.date) >= oneMonthAgo
    );

    if (recentPayouts.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No recent payouts found" });
    }
    ...
    const csvData =
      ...
      csvStringifier.stringifyRecords(recentPayouts);

    res.setHeader("Content-Type", "text/csv");
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=recent_payouts.csv"
    );
...
export const getAllUsersTotalPayoutSum = async (
  req: Request,
  res: Response
) => {
  try {

...
// @access  Private (User)
export const cancelWithdrawal = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const payout = user.payouts.id(payoutId);
        ...
        if (payout.status !== 'Pending') {
            await session.abortTransaction();
            session.endSession();
            return next(new ErrorHandler(`Cannot cancel a payout with status '${payout.status}'.`, 400));
        }
        ...
        res.status(200).json({
            success: true,
            message: "Withdrawal cancelled successfully. Funds have been returned to your earnings balance.",
...
export const withdrawFunds = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (!amount || amount <= 0 || !bankName) { 
            await session.abortTransaction();
            session.endSession();
            return next(new ErrorHandler("Invalid amount or missing bank details.", 400));
        }
